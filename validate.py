...
def compute_edge_matching_accuracy(grid, pieces, truth):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # look at left neighbor and right neighbor
            if c + 1 < cols:
                ...
                if right_edge.size > 0 and left_edge.size > 0:
                    ...
                    if len(right_flat) > 1 and len(left_flat) > 1 and np.std(right_flat) > 0 and np.std(left_flat) > 0:
                        ...
                        # correct if correlation exceeds threshold
                        if np.isfinite(edge_corr) and edge_corr > 0.5:
                            correct_matches += 1
            
            # look at bottom neighbor and top neighbor
            if r + 1 < rows:
    ...
    # ratio
    edge_match_accuracy = correct_matches / max(total_edges, 1)
...
def compute_piece_placement_accuracy(reconstructed, truth, grid, pieces):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # compare the reconstructed and original
            reconstructed_region = reconstructed[r*piece_size:(r+1)*piece_size, c*piece_size:(c+1)*piece_size]
            truth_region = truth[r*piece_size:(r+1)*piece_size, c*piece_size:(c+1)*piece_size]
            
            if reconstructed_region.shape == truth_region.shape:
                reconstructed_flat = reconstructed_region.ravel().astype(float)
                truth_flat = truth_region.ravel().astype(float)
                
                # normalize
                reconstructed_std = np.std(reconstructed_flat)
                truth_std = np.std(truth_flat)
                
                if reconstructed_std > 0 and truth_std > 0:
                    reconstructed_flat = (reconstructed_flat - np.mean(reconstructed_flat)) / reconstructed_std
                    truth_flat = (truth_flat - np.mean(truth_flat)) / truth_std
                    
                    # compute correlation and similarity
                    region_corr = np.corrcoef(reconstructed_flat, truth_flat)[0, 1]
                    
                    # if placement is correct the correlation will be higher
                    if np.isfinite(region_corr) and region_corr > 0.6:
...
def validate_reconstruction(reconstructed, truth_image_path, grid=None, pieces=None, puzzle_name=None, iteration=None, logger=None):
    ...
    # correlation
    reconstructed_norm = reconstructed.astype(float) / 255.0
    truth_norm = truth.astype(float) / 255.0
    correlation = np.corrcoef(reconstructed_norm.ravel(), truth_norm.ravel())[0, 1]
    
    # histogram
    histogram_scores = []
    for i in range(3):
        histogram_reconstructed = np.histogram(reconstructed[:,:,i].ravel(), bins=256, range=(0, 256))[0].astype(float)
        histogram_truth = np.histogram(truth[:,:,i].ravel(), bins=256, range=(0, 256))[0].astype(float)
        histogram_reconstructed = histogram_reconstructed / (np.sum(histogram_reconstructed) + 1e-5)
        histogram_truth = histogram_truth / (np.sum(histogram_truth) + 1e-5)
        score = np.corrcoef(histogram_reconstructed, histogram_truth)[0, 1]
        if np.isfinite(score):
            histogram_scores.append(score)
    avg_histogram_score = np.mean(histogram_scores) if histogram_scores else 0.0
    
    # edge matching accuracy ratio
    edge_match_accuracy = compute_edge_matching_accuracy(grid, pieces, truth) if grid and pieces else 0.0
    
    # piece placement accuracy ratio
    ...
    results = {
        ...
        'hist_score': avg_histogram_score,
    ...
    if puzzle_name and iteration and logger:
        ...
        log(f"Histogram Match: {avg_histogram_score:.4f}")
